...
<head>
    ...
    <title>Editar Cámara | UTS Redes</title>
...
<body class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-2xl">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold text-white mb-2">Editar Cámara</h1>
            <p class="text-gray-400">Actualiza los datos de la cámara</p>
        </div>

        <div class="bg-white/10 backdrop-blur-2xl rounded-2xl p-8 shadow-2xl border border-white/20">
            <form action="/camaras/{{ $camara->id }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="ubicacion" value="{{ $camara->ubicacion }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="ip" value="{{ $camara->ip }}" required
                            class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition">
                        @error('ip')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="mac" value="{{ $camara->mac }}"
                            class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition">
                        @error('mac')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="nvr" value="{{ $camara->nvr }}"
                            class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition">
                        @error('nvr')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="canal" value="{{ $camara->canal }}"
                            class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition">
                        @error('canal')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="flex gap-4 pt-6 border-t border-white/10">
                    <button type="submit" class="flex-1 px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-600 hover:from-blue-600 hover:to-cyan-700 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition inline-flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
